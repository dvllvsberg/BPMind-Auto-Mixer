...
## [1.8.0-beta.3] — 2026-07-01

Beta для внешних тестировщиков: плавность play, tape_stop, UI таймлайна.

### Плавность воспроизведения и UI

- Preload переходов: более ранний warmup (до 8+ с), блокировка только у конца сегмента
- План таймлайна кэширует intro_skip / точку входа — без повторного скана диска при preload
- Пауза: `sleep` вне lock — UI не блокируется при ожидании
- Таймлайн: статический слой в pixmap, playhead обновляется раз в секунду
- UI-таймер замедляется в фоне (1.2 с); меньше лишних repaint при возврате в окно
- Плейлист/метки обновляются только при смене трека, не каждые 400 ms
- tape_stop: prefetch эффекта и входящего main при старте сета + in-memory кэш
- Диалог конца сета немодальный — таймлайн продолжает двигаться, пока открыто окно
- Перемотка по таймлайну: частичная загрузка с позиции seek (не весь трек с диска)

### Производительность и стабильность

- Скан библиотеки: лёгкий lookup без гидратации + один commit на весь скан
- SQLite WAL и `busy_timeout` — меньше блокировок при параллельном доступе
- `get_by_ids()` — bulk-загрузка треков для плейлиста/микса (3 запроса вместо N×3)
- Quick-анализ: декодирование сразу в 22.05 kHz
- Play: переиспользование уже построенного таймлайна (без повторного I/O)
- Экспорт блокируется во время сканирования
- Плеер: лог ошибок воспроизведения вместо молчаливого пропуска трека
...
- За ~45 с до конца (или 15% длины сета): «Микс подходит к концу. Продолжим вечеринку?»
- **Пересобрать** — остановка и новый микс
- **Продолжить** — сет по кругу с переходами (`SessionPlayer.loop_session`)
- **Завершить** — доиграть до конца без повторного вопроса
