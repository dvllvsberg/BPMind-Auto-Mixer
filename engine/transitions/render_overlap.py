...
from engine.transitions.tape_stop import TAPE_STOP_SR, tape_motor_stop_outgoing
...
def render_transition_overlap(
  ...
  if transition_is_solo_tail(transition.type):
    if outgoing_tail.size == 0:
      ...
      return _append_tape_silence(silence, outgoing_tape_silence_sec(transition))
    processed = tape_motor_stop_outgoing(outgoing_tail)
    return _append_tape_silence(processed, outgoing_tape_silence_sec(transition))

  return mix_transition_segments(transition.type, outgoing_tail, incoming_head)
